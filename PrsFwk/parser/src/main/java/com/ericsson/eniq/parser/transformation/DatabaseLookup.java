...
public class DatabaseLookup implements Transformation {
  ...
  public void transform(final Map data, Logger logger) {
    ...
    if (logger.isDebugEnabled()) {
    	logger.debug("DatabaseLookup.transform: lookupMap = " + lookupMap);
    	logger.debug("DatabaseLookup.transform: sql = " + sql);
    	logger.debug("DatabaseLookup.transform: data = " + data);
    	logger.debug("DatabaseLookup.transform: tgt = " + tgt);
    }
    
    final String lookedup = (String) lookupMap.get(data.get(src));

    logger.debug("DatabaseLookup.transform: lookedup = " + lookedup);
  ...
  public void configure(final String name, final String src, final String tgt, final Properties props, final Logger logger)
    ...
    try {
      ...
      logger.debug("basetable: " + basetable);
      ...
      if (basetable == null) {
        ...
        for (int i = 0; i < basetables.length; i++) {
          ...
          logger.debug("Adding SQL " + sql + " to table " + basetables[i]);
        }

      }

    } catch (Exception e) {
    	logger.warn("DatabaseLookup: Error while configuring: "+sql,e);
    }
