...
public class DateFormat implements Transformation {
  ...
  public void transform(final Map data, final Logger log) {
    ...
    log.debug("DateFormat: TimeZone " + t_oldTimezone + " -> " + t_newTimezone);
    ...
    try {
      ...
      log.debug("DateFormat: Format " + t_oldFormat + " -> " + t_newFormat);
      ...
      log.debug("DateFormat: Locale " + t_oldLocale + " -> " + t_newLocale);
      ...
      if (null != cached) {
        log.debug("DateFormat.transform: Found date from cache. Value = " + cached);
        data.put(tgt, cached);
        return;
      }
      ...
      log.info("DateFormat: Error while figuring out parameters.", e);
    }
    ...
    try {
      ...
      log.debug("DateFormat.transform: input " + input + " transformed to output " + output);
      ...
      log.debug("DateFormat: Invalid timestamp: \"" + input + "\" in field " + src);
    }
