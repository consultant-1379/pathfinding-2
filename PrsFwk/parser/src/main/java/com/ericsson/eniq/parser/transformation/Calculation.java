...
public class Calculation implements Transformation {
  ...
  public void transform(final Map data, final Logger logger) {
    ...
    if (useFloat) {
      ...
      try {
        ...
        logger.debug("CalculationTransformation: Source " + tinput + " is not a float");
        return;
      }
      ...
      if (operandName != null) {
        ...
        try {
          operand = Double.parseDouble(toperand);
        	...
        	logger.debug("CalculationTransformation: Operand " + toperand + " is not a float");
          return;
        }
      }
      ...
      try {
        switch (operation) {
        case 30:
        	logger.warn("For Calculation of PMRes Quantity Measurment, double precision should not " +
        ...
        logger.debug("Calculation error", e);
      }
      ...
      try {
        ...
        logger.debug("CalculationTransformation: Source " + tinput + " is not an integer");
        return;
      }
      ...
      if (operandName != null) {
        ...
        try {
          ...
          logger.debug("CalculationTransformation: Operand " + toperand + " is not an integer");
          return;
        }
      }
  ...
  public void configure(final String name, final String src, final String tgt, final Properties props, final Logger logger)
